<script lang="ts">
	...
	let filteredContacts = $derived(
		appContacts.filter(
			(c) =>
				...
				c.email.toLowerCase().includes(shareSearchQuery.toLowerCase()),
		),
	);
	...
	async function loadNotes() {
		...
		if (data) {
			notes = data.map((n) => {
				const sharedUsers = (n.note_shares || [])
					.map((share: any) => {
						const contact = appContacts.find(
							(c) => c.id === share.shared_with,
						);
						return contact
							? { avatar: contact.avatar, color: contact.color }
							: null;
					})
					.filter(Boolean);
...
<div
	class="flex-1 flex h-full overflow-hidden bg-[#070b0e] text-brand-text relative"
>
	...
	>
		<div class="p-6 pb-2 flex-1 relative">
			<div class="flex items-center justify-between mb-4">
				<h2
					class="text-[10px] font-bold text-brand-text-muted tracking-widest uppercase"
				>
					Carpetas
				</h2>
				<button
					class="md:hidden p-1 rounded-md text-brand-text-muted hover:text-brand-text transition-colors"
			...
			<button
				class="hidden md:block absolute top-5 right-4 p-1 rounded-md text-brand-text-muted hover:text-brand-text transition-colors"
			...
			<ul class="space-y-1 mt-4">
				<li>
					<button
						class="w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm font-semibold transition-colors {activeFolderId ===
							null && activeFolderName === 'Todas las Notas'
							? 'bg-brand-surface-elevated text-brand-text border border-brand-divider'
					...
					>
						<div class="flex items-center gap-3">
							<Folder
								class="w-4 h-4 {activeFolderId === null &&
								activeFolderName === 'Todas las Notas'
									? 'text-brand-accent'
				...
				<li>
					<button
						class="w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm font-semibold transition-colors {activeFolderId ===
							null && activeFolderName === 'Favoritas'
							? 'bg-brand-surface-elevated text-brand-text border border-brand-divider'
							: 'text-brand-text-muted hover:text-brand-text hover:bg-brand-surface'}"
						onclick={() => {
							activeFolderId = null;
							activeFolderName = "Favoritas";
							showMobileFolders = false;
						}}
					>
						<div class="flex items-center gap-3">
				...
				{#each folders as folder}
					...
					<li
						class="group flex items-center justify-between px-3 py-2 rounded-xl text-sm font-semibold transition-colors {activeFolderId ===
						folder.id
							? 'bg-brand-surface-elevated text-brand-text border border-brand-divider'
							: 'text-brand-text-muted hover:bg-brand-surface'} cursor-pointer"
						onclick={() => {
					...
					>
						<div class="flex items-center gap-3 truncate">
							<Folder
								class="w-4 h-4 shrink-0 {activeFolderId ===
								folder.id
									? 'text-brand-accent'
									: 'text-brand-text-muted'}"
							/>
							<span
								class="truncate {activeFolderId === folder.id
									? 'text-brand-text'
						...
						<div
							class="flex items-center gap-1 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 focus-within:opacity-100 transition-opacity shrink-0"
						>
							<button
								class="p-1 rounded text-brand-text-muted hover:text-brand-text hover:bg-brand-divider transition-colors"
							...
							<button
								class="p-1 rounded text-brand-text-muted hover:text-red-400 hover:bg-red-400/10 transition-colors"
		...
		<div
			class="p-6 border-t border-brand-divider bg-[#070b0e] sticky bottom-0"
		>
			<button
				class="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-brand-divider border-dashed text-brand-text-muted hover:text-brand-text hover:border-brand-accent transition-colors text-xs font-bold shadow-inner bg-brand-surface/30"
				onclick={openNewFolderModal}
			>
	...
	>
		...
		<div class="p-6 pb-4 shrink-0 bg-[#0d1216] z-10 sticky top-0">
			<div class="flex items-center justify-between mb-6">
				<div class="flex items-center gap-3 truncate">
					<button
						class="md:hidden p-1.5 rounded-md text-brand-text-muted hover:text-brand-text hover:bg-brand-surface transition-colors shrink-0"
						onclick={() => (showMobileFolders = true)}
					>
						<Menu class="w-4 h-4" />
					</button>
					<h2
						class="text-xl font-bold text-brand-text flex items-center gap-2 truncate"
					>
						{activeFolderName}
					</h2>
				</div>
				<div class="flex items-center gap-2 shrink-0">
					<div
						class="flex items-center gap-1 bg-[#070b0e] p-1 rounded-lg border border-brand-divider"
					>
						<button
							class="p-1 rounded-md transition-colors {viewMode ===
							'list'
								? 'bg-brand-surface-elevated text-brand-accent border border-brand-divider shadow-sm'
					...
					<button
						class="md:hidden p-1.5 rounded-md text-brand-text-muted hover:text-brand-text transition-colors"
		...
		<div class="flex-1 overflow-y-auto custom-scrollbar px-4 pb-6">
				...
				<div class="space-y-3 mb-8">
					{#each filteredNotes.filter((n) => n.pinned) as note}
						<button
							...
							'grid'
								? 'p-4'
								: 'p-3'} border transition-colors group {note.id ===
							activeNoteId
								? 'border-brand-accent shadow-[0_0_15px_var(--color-brand-accent-muted)]'
						...
						>
							<div
								class="flex items-start justify-between {viewMode ===
								'grid'
									? 'mb-2'
									: ''}"
							>
								<h4
									class="text-sm font-bold truncate pr-4 {note.id ===
									activeNoteId
										? 'text-brand-text'
								...
								<Star
									class="w-3.5 h-3.5 {note.pinned
										? 'text-amber-400'
										: 'text-brand-text-muted opacity-0 group-hover:opacity-100'} shrink-0"
							...
							{#if viewMode === "grid"}
								<p
									class="text-[11px] text-brand-text-muted leading-relaxed line-clamp-2 mb-3"
								>
									{note.excerpt
										.replace(/<[^>]+>/g, " ")
										.replace(/\s+/g, " ")
									...
									{#if note.sharedUsers && note.sharedUsers.length > 0}
										...
										>
											{#each note.sharedUsers.slice(0, 3) as user}
												<div
													class="w-5 h-5 rounded-full {user.color} flex items-center justify-center text-white text-[8px] font-bold border border-[#0d1216] shadow-sm relative z-10"
													title="Compartido"
												>
													{user.avatar}
												</div>
											...
											{#if note.sharedUsers.length > 3}
												<div
													class="w-5 h-5 rounded-full bg-brand-surface border border-brand-divider flex items-center justify-center text-brand-text-muted text-[8px] font-bold z-0"
												>
													+{note.sharedUsers.length -
														3}
												</div>
											{/if}
										</div>
							...
							{:else}
								<div
									class="flex items-center justify-between text-[10px] font-medium mt-1"
								>
				...
				<div class="space-y-3 mt-4">
					{#each filteredNotes.filter((n) => !n.pinned) as note}
						<button
							...
							activeNoteId
								? 'border-brand-accent shadow-[0_0_15px_var(--color-brand-accent-muted)]'
								: 'border-brand-divider hover:border-brand-accent/50 shadow-sm'}"
							onclick={() => {
								activeNoteId = note.id;
								showMobileList = false;
							}}
						>
							...
							>
								<div class="min-w-0 pr-4">
									<h4
										class="text-sm font-bold truncate {note.id ===
										activeNoteId
											? 'text-brand-text'
											: 'text-brand-text group-hover:text-brand-accent'}"
									>
										{#if note.isLocked}<Lock
												class="w-3 h-3 inline-block mr-1 text-orange-400"
											/>{/if}{note.title}
									</h4>
								</div>
								{#if note.status && viewMode === "grid"}
									<span
										class="text-[9px] font-bold px-1.5 py-0.5 rounded border whitespace-nowrap {note.statusColor
											? `${note.statusColor.replace('text-', 'bg-')}/10 border-${note.statusColor.replace('text-', '')}/20 ${note.statusColor}`
							...
							{:else}
								...
								>
									...
									<div class="flex gap-2 items-center">
										...
										{#if note.status}
											<span
												class="font-bold {note.statusColor ||
													'text-brand-text'}"
												>{note.status}</span
											>
										{/if}
			...
			{#if filteredNotes.length === 0}
				<div
					class="text-center py-10 text-brand-text-muted text-sm font-medium"
				>
	...
	<main class="flex-1 flex flex-col bg-[#070b0e] h-full relative z-0">
		<!-- Editor Topbar -->
		<header
			class="shrink-0 h-16 border-b border-brand-divider px-6 flex items-center justify-between"
		>
			<div class="flex items-center gap-3">
				<button
					class="p-2 rounded-lg text-brand-text hover:bg-brand-surface border border-brand-divider transition-colors md:hidden"
				...
				<div
					class="hidden sm:flex items-center gap-2 text-xs font-bold text-brand-accent bg-brand-accent/5 px-3 py-1.5 rounded-lg border border-brand-accent/20"
				>
				...
				<div
					class="h-4 w-px bg-brand-divider mx-2 hidden sm:block"
				></div>
				<span
					class="flex items-center gap-1.5 text-[10px] font-semibold text-brand-text-muted"
				>
					<span
						class="w-1.5 h-1.5 rounded-full bg-brand-text-muted/50"
					></span>
					{lastEditedText}
				</span>
			</div>
			...
			<div class="flex items-center gap-3">
				...
				{#if activeNote}
					<button
						class="px-3 py-1.5 rounded-lg text-[11px] font-bold text-brand-bg bg-brand-accent hover:brightness-110 transition-colors shadow-[0_0_15px_var(--color-brand-accent-muted)] flex items-center gap-2"
						onclick={saveActiveNote}
					>
						<Check class="w-3.5 h-3.5" strokeWidth={3} /> Guardar
					</button>
					<button
						class="p-2 rounded-lg transition-colors shadow-sm {activeNote.pinned
							? 'text-amber-400 bg-amber-400/10 border border-amber-400/20 hover:bg-amber-400/20'
							: 'text-brand-text-muted bg-[#0d1216] border border-brand-divider hover:text-brand-text'}"
						onclick={toggleActiveNotePinned}
						title={activeNote.pinned
							? "Desanclar nota"
							: "Anclar nota"}
					>
						<Star
					...
					<button
						class="p-2 rounded-lg text-brand-text-muted bg-[#0d1216] border border-brand-divider hover:text-brand-text transition-colors shadow-sm"
						onclick={shareActiveNote}
						title="Compartir nota"
					>
						<Share2 class="w-4 h-4" />
					</button>
					<button
						class="p-2 rounded-lg text-red-400/70 bg-[#0d1216] border border-brand-divider hover:text-red-400 hover:bg-red-400/10 transition-colors shadow-sm"
			...
			{#if activeNote}
				<div class="max-w-3xl mx-auto px-8 py-12 pb-32">
					...
					>
						...
						>
							<button
								class="ql-bold p-2 rounded-lg text-brand-text hover:bg-brand-surface transition-colors focus:outline-none"
								title="Negrita"><Bold class="w-4 h-4" /></button
							>
							<button
								class="ql-italic p-2 rounded-lg text-brand-text hover:bg-brand-surface transition-colors focus:outline-none"
								title="Cursiva"
								><Italic class="w-4 h-4" /></button
							>
							<button
								class="ql-underline p-2 rounded-lg text-brand-text hover:bg-brand-surface transition-colors focus:outline-none"
								title="Subrayado"
								><Underline class="w-4 h-4" /></button
							>
						</div>
						<div
							class="flex items-center gap-1 pl-2 pr-2 border-r border-brand-divider"
						>
							<button
								class="ql-header p-2 rounded-lg text-brand-text hover:bg-brand-surface transition-colors focus:outline-none"
								value="1"
								title="Título 1"
						...
						>
							...
							<button
								class="ql-align p-2 rounded-lg text-brand-text hover:bg-brand-surface transition-colors focus:outline-none"
								value="center"
								title="Alineación Centro"
								><AlignLeft class="w-4 h-4" /></button
							>
							<button
								class="ql-code-block p-2 rounded-lg text-brand-text hover:bg-brand-surface transition-colors focus:outline-none"
								title="Código"><Code class="w-4 h-4" /></button
							>
							<button
								type="button"
								class="p-2 rounded-lg text-brand-text hover:bg-brand-surface transition-colors focus:outline-none flex items-center justify-center"
						...
						<div class="flex items-center gap-1 pl-2">
							<button
								class="ql-blockquote px-3 py-1.5 flex items-center gap-2 rounded-lg text-amber-400 hover:bg-amber-400/10 text-xs font-bold transition-colors focus:outline-none"
								title="Destacado"
								><Target class="w-4 h-4" /> Idea clave</button
							>
						</div>
					</div>
			...
			{:else}
				<div class="flex items-center justify-center h-full">
					<div class="text-center">
						<div
							class="w-16 h-16 rounded-2xl bg-brand-surface border border-brand-divider flex items-center justify-center mx-auto mb-4 shadow-sm text-brand-text-muted"
						>
						...
						<p class="text-sm font-medium text-brand-text-muted">
							Selecciona una nota de la lista para ver su
							contenido.
						</p>
					</div>
				</div>
...
{#if showNewFolderModal}
	...
	<div
		...
		onclick={(e) => {
			if (e.target === e.currentTarget) showNewFolderModal = false;
		}}
	>
		<div
			class="bg-[#0d1216] border border-brand-divider rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl shadow-black"
		>
			<div class="p-6">
				...
				<input
					...
					onkeydown={(e) => {
						...
						if (e.key === "Escape") showNewFolderModal = false;
					}}
					autofocus
				/>
			</div>
			<div
				class="px-6 py-4 bg-brand-surface/30 border-t border-brand-divider flex justify-end gap-3"
			>
				<button
...
{#if showShareToast}
	<div
		class="fixed top-24 right-10 bg-brand-accent text-brand-bg px-6 py-3 rounded-xl shadow-[0_0_20px_var(--color-brand-accent-muted)] z-50 flex items-center gap-3 font-bold text-sm transition-all duration-300"
	>
...
{#if showDeleteModal}
	...
	>
		...
		>
			<div class="p-6">
				<div
					class="w-12 h-12 rounded-full bg-red-400/10 flex items-center justify-center mb-4"
				>
				...
				<p class="text-sm font-medium text-brand-text-muted">
					¿Estás seguro de que deseas eliminar la nota <span
						class="text-brand-text font-bold"
...
{#if showSaveToast}
	<div
		class="fixed top-10 right-10 bg-brand-accent text-brand-bg px-6 py-3 rounded-xl shadow-[0_0_20px_var(--color-brand-accent-muted)] z-50 flex items-center gap-3 font-bold text-sm transition-all duration-300"
	>
	...
	>
		...
		>
			<div class="p-6 pb-4 border-b border-brand-divider">
				<div
					class="w-12 h-12 rounded-full bg-brand-accent/10 flex items-center justify-center mb-4"
				>
					<Share2 class="w-6 h-6 text-brand-accent" />
				</div>
				<h3 class="text-lg font-bold text-brand-text mb-1">
					Compartir nota
				</h3>
				<p class="text-sm font-medium text-brand-text-muted">
			...
			<div class="p-4 bg-brand-surface/30 border-b border-brand-divider">
				<div class="relative">
					<Search
						class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-text-muted"
					/>
					<input
						type="text"
			...
			<div class="max-h-60 overflow-y-auto custom-scrollbar p-2">
				{#each filteredContacts as contact}
					...
					<!-- svelte-ignore a11y_no_static_element_interactions -->
					<div
						class="flex items-center justify-between p-3 rounded-xl hover:bg-brand-surface cursor-pointer transition-colors"
						onclick={() => (contact.selected = !contact.selected)}
					>
						<div class="flex items-center gap-3">
							<div
								class="w-10 h-10 rounded-full {contact.color} text-white font-bold flex items-center justify-center shadow-sm"
							>
								{contact.avatar}
							</div>
							<div>
								...
								<p
									class="text-[10px] text-brand-text-muted font-medium"
								>
									{contact.email}
								</p>
							</div>
						...
						<div
							class="w-5 h-5 rounded border {contact.selected
								? 'bg-brand-accent border-brand-accent'
								: 'bg-transparent border-brand-divider'} flex items-center justify-center transition-colors"
						>
							{#if contact.selected}
								<Check
									class="w-3.5 h-3.5 text-brand-bg"
									strokeWidth={3}
								/>
							{/if}
						</div>
				...
				{#if filteredContacts.length === 0}
					<div
						class="text-center py-6 text-brand-text-muted text-sm font-medium"
					>
			...
			<div
				class="px-6 py-4 bg-brand-surface/30 border-t border-brand-divider flex justify-between items-center"
			>
				...
				<div class="flex gap-3">
					...
					<button
						class="px-4 py-2 rounded-xl text-sm font-bold text-brand-bg transition-colors shadow-sm {appContacts.some(
							(c) => c.selected,
						)
							? 'bg-brand-accent hover:brightness-110 shadow-[0_0_15px_var(--color-brand-accent-muted)]'
							: 'bg-[#070b0e] border border-brand-divider text-brand-text-muted cursor-not-allowed'}"
	...
	>
		...
		>
			...
			<p class="text-sm text-brand-text-muted mb-6">
				Las notas dentro de "{folderToDelete?.name}" no se eliminarán,
				pero dejarán de pertenecer a esta carpeta.
			</p>
			<div class="flex gap-3">
				<button
					class="flex-1 py-3 rounded-xl font-bold text-sm text-brand-text-muted hover:text-brand-text transition-colors"
