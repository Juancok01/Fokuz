	...
	onMount(() => {
		...
		if (taskIdRaw && title) {
			...
			if (!Number.isNaN(id)) {
				...
				pomodoroUI.isMaximized = true; // Auto open if navigating via task link
			}
		}
	});

	afterNavigate(() => {
		pomodoroUI.isMaximized = false;
	});

	let showWidget = $derived(
		!pomodoroUI.isMaximized && 
...
{#if pomodoroUI.isMaximized}
	<div class="absolute inset-0 z-40 bg-[#070b0e] flex flex-col animate-in fade-in zoom-in-95 duration-200">
		...
		<!-- Header bar for Maximized view to allow minimizing -->
		<div class="sticky top-0 z-[110] bg-[#070b0e]/90 backdrop-blur-md border-b border-brand-divider p-4 flex items-center justify-between px-6">
			<div class="flex items-center gap-3">
				...
				<span class="font-bold text-brand-text">Fokuz Pomodoro</span>
			</div>
			<button class="p-2 rounded-xl hover:bg-brand-surface border border-transparent hover:border-brand-divider text-brand-text-muted hover:text-brand-text transition-all flex items-center gap-2 text-xs font-bold" onclick={() => pomodoroUI.isMaximized = false}>
				<Minimize2 class="w-4 h-4" /> Minimizar
			</button>
		</div>

		<!-- Main Content Area -->
		<div class="flex-1 overflow-y-auto custom-scrollbar px-4 py-6 md:px-8 pb-32 w-full max-w-7xl mx-auto">
			...
			<div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
				...
				<div class="lg:col-span-8 flex flex-col gap-6">
					...
					<div class="bg-[#0d1216] border border-brand-divider rounded-4xl p-8 flex flex-col items-center relative overflow-hidden shadow-2xl h-105">
						...
						<h2 class="text-[11px] font-black {pomodoro.awaitingAck ? 'text-amber-400' : 'text-brand-accent'} tracking-[0.2em] uppercase z-10 mb-8">
						...
						<div class="relative w-64 h-64 z-10">
							<svg class="w-full h-full -rotate-90" viewBox="0 0 120 120" aria-hidden="true">
								...
								<circle 
									...
									class="transition-[stroke-dashoffset] duration-1000 {pomodoro.awaitingAck ? 'text-amber-400 drop-shadow-[0_0_15px_rgba(251,191,36,0.5)]' : 'text-brand-accent drop-shadow-[0_0_15px_var(--color-brand-accent)]'}" 
									style="filter: drop-shadow(0px 0px 8px {pomodoro.awaitingAck ? 'rgba(251,191,36,0.4)' : 'var(--color-brand-accent-muted)'});"
							...
							<div class="absolute inset-0 flex flex-col items-center justify-center">
								...
								<span class="text-[10px] font-bold uppercase tracking-widest mt-2 {pomodoro.awaitingAck ? 'text-amber-400' : 'text-brand-text-muted'}">
						...
						<div class="absolute bottom-8 z-20 flex items-center justify-center gap-6">
							...
							{#if pomodoro.awaitingAck}
								<button class="w-16 h-16 rounded-full bg-amber-400 text-[#070b0e] flex items-center justify-center hover:scale-105 transition-all shadow-[0_0_20px_rgba(251,191,36,0.5)]" onclick={acknowledgePomodoro}>
...
{:else if showWidget}
	...
	<div class="fixed bottom-20 md:bottom-6 right-4 md:right-8 z-40 bg-[#0d1216]/90 backdrop-blur-md border border-brand-divider rounded-full pr-2 pl-6 py-2 shadow-2xl flex items-center gap-4 animate-in slide-in-from-bottom-5">
		<!-- Mini timer text -->
		<div class="flex flex-col items-center justify-center min-w-[60px]">
			<span class="text-[8px] font-bold {pomodoro.awaitingAck ? 'text-amber-400' : 'text-brand-accent'} uppercase tracking-wider">{pomodoro.phase === 'focus' ? 'Enfoque' : 'Descanso'}</span>
			<span class="text-lg font-black text-brand-text tabular-nums leading-none">{formatPomodoroTime(pomodoro.remainingMs)}</span>
		</div>
		
		<div class="w-px h-8 bg-brand-divider mx-1"></div>
		
		<div class="flex items-center gap-0.5 text-brand-text-muted">
			<button class="p-2 hover:text-brand-text hover:bg-brand-surface-elevated rounded-full transition-colors group" onclick={() => pomodoroUI.isMaximized = true} title="Abrir música">
				<Music class="w-3.5 h-3.5 {pomodoro.awaitingAck ? 'text-amber-400' : 'group-hover:text-brand-accent'} transition-colors" />
			</button>
			
			<button class="p-2 hover:text-brand-text hover:bg-brand-surface-elevated rounded-full transition-colors" onclick={() => { if (!ytPlayer.currentPlaylistId && ytPlayer.playlists.length > 0) ytPlayer.playTrack(ytPlayer.playlists[0].id, 0); else ytPlayer.togglePlay(); }} title="Reproducir/Pausar música">
				{#if ytPlayer.isPlaying}<Pause class="w-3.5 h-3.5" fill="currentColor" />{:else}<Play class="w-3.5 h-3.5" fill="currentColor" />{/if}
			</button>
		...
		<div class="flex items-center gap-2">
			{#if pomodoro.awaitingAck}
				<button class="w-10 h-10 rounded-full bg-amber-400 text-[#070b0e] flex items-center justify-center hover:scale-105 transition-transform shadow-[0_0_10px_rgba(251,191,36,0.5)]" onclick={acknowledgePomodoro}>
					<Check class="w-5 h-5" strokeWidth={3} />
				</button>
			{:else if pomodoro.running}
				<button class="w-10 h-10 rounded-full bg-brand-accent text-brand-bg flex items-center justify-center hover:scale-105 transition-transform shadow-[0_0_10px_var(--color-brand-accent-muted)]" onclick={pausePomodoro}>
					<Pause class="w-5 h-5" fill="currentColor" />
				</button>
			{:else}
				<button class="w-10 h-10 rounded-full bg-brand-accent text-brand-bg flex items-center justify-center hover:scale-105 transition-transform shadow-[0_0_10px_var(--color-brand-accent-muted)]" onclick={startPomodoro}>
					<Play class="w-5 h-5 ml-1" fill="currentColor" />
				</button>
			{/if}

			<button class="w-10 h-10 flex items-center justify-center rounded-full text-brand-text-muted hover:text-brand-text hover:bg-brand-surface-elevated transition-colors" onclick={() => pomodoroUI.isMaximized = true}>
